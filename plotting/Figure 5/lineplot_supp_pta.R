...
ode_baseline <- read_csv("../ode_baseline_pta.csv") |>
  select(rotifers_algae_coherent, lynx_hares) |>
  pivot_longer(everything(), names_to = "experiment", values_to = "mean_pta_norm") |>
...
results_tl <- read_csv("real_world_noise_experiments_adjusted.csv") |>
  select(pta=`1-PTA`, noise_std, experiment, noise_type) |>
  group_by(experiment, noise_type, noise_std) |>
  summarise(mean_pta = mean(pta)) 
...
results_tl_clean <- results_tl |>
  ...
  select(experiment, noise_type, noise_level=noise_std.x, mean_pta=mean_pta.x, dl_baseline=mean_pta.y) |>
  mutate(mean_pta_norm = mean_pta/dl_baseline) |>
...
dl_baseline <- results_tl |>
  ...
  mutate(noise_type = "DL baseline",
         noise_level="0.0",
         mean_pta_norm = mean_pta/mean_pta) |>
  select(-c("noise_std", "mean_pta"))
...
# plots figure
(fig <- ggplot(data=df_combined, aes(x=as.factor(noise_type_rank), y=mean_pta_norm, group=noise_type, color=noise_type)) +
  ...
  theme(legend.position = "top",
   ...
   ggsave("supp_noise_pta_v1.pdf", fig, width=15, height=6, device=cairo_pdf)
