...
experiment_directory = os.getcwd() + "/src/experiments/julian/algae-rotifers/run-1/"
    ...
    for folder in folders:
        if str(folder).startswith("TS") and not str(folder).startswith("TS10000"):
            output_path = os.path.join(
                "src/experiments/julian/algae-rotifers/run-1/runs_transfer_learning", folder, "transfer.csv"
            )
            config_path = os.path.join(
                "src/experiments/julian/algae-rotifers/run-1/runs_transfer_learning",
                folder,
                "transfer.toml",
            )
            script_lines.append(
