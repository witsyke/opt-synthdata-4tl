...
experiment_directory = os.getcwd() + "/src/experiments/julian/covid/run-6/"
    ...
    for folder in folders:
        if str(folder).startswith("TS"):
            output_path = os.path.join(
                "src/experiments/julian/covid/run-6/runs_transfer_learning", folder, "transfer.csv"
            )
            config_path = os.path.join(
                "src/experiments/julian/covid/run-6/runs_transfer_learning",
                folder,
                "transfer.toml",
            )
            script_lines.append(
