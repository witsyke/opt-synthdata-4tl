...
def generate_bash_script(folders):
    ...
    for folder in folders:
        if not str(folder).startswith("TS10000"):
            output_path = os.path.join(
                "src/experiments/julian/lynx-hares/run-10/runs_transfer_learning", folder, "transfer.csv"
            )
            config_path = os.path.join(
                "src/experiments/julian/lynx-hares/run-10/runs_transfer_learning",
                folder,
                "transfer.toml",
            )
            script_lines.append(
