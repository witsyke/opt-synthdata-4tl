...
class _GRUNeuralNetwork(pl.LightningModule):  # pylint: disable=too-many-ancestors
    def __init__(
        ...
        self.gru0 = nn.GRU(input_size=len(time_series_params.input_features),
                          hidden_size=128,
                          batch_first=True,
                          num_layers=1,)
        self.gru1n2 = nn.GRU(input_size=128,
                          hidden_size=128,
                          batch_first=True,
                          num_layers=1,)
        self.fc3 = nn.Linear(
    ...
    def configure_optimizers(self) -> torch.optim.Adam:
        if self.model_params.finetuning:
            ...
            for name, param in self.named_parameters():
                if "gru0" in name or "gru1n2" in name:
